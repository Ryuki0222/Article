...
const viewTableItemButtonShowStyle = {
    position: 'relative',
...
export default class MyArticle extends Component {
    ...
    onClick(e, id) {
        ReactDOM.render(<EditArticle id={id} />, document.querySelector('.primary_body'))
    }

    componentWillMount() {
        const userId = Cookie.get(Define.COOKIE_USER_ID_KEY())
        axios.get(`/api/v1/user/${userId}/articles`)
            .then(res => {
                this.setState({
						...
						<tbody>
                            {
                                this.state.articles.map(article => {
                                    return <ViewTableRow article={article} button={'Edit'} onClick={e => this.onClick(e, article.id)}/>
                                })
                            }
						</tbody>
					</table>
